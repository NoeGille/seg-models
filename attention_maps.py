...
def plot_attention_map(block, fig, attention_maps, image = None):
    ...
    min = attention_maps[0][block][1:, 1:].flatten().min()
    max = attention_maps[0][block][1:, 1:].flatten().max()
    ...
    for i in range(196):
        ...
        plt.imshow(first_block_attention_maps[i, :].reshape(14, 14).cpu().detach().numpy(), alpha=0.5, vmin=min, vmax=max, cmap='jet')
        plt.axis('off')
...
def plot_attention_rollout(block, fig, attention_rollout_maps, image=None):
    ...
    min = nth_block_rollout.flatten().min()
    max = nth_block_rollout.flatten().max()
    if image != None:
        plt.imshow(image.permute(1,2,0).cpu().detach().numpy(), cmap='gray', alpha=1)
    for j in range(196):
        fig.add_subplot(14, 14, j+1)
        plt.imshow(nth_block_rollout[j, :].reshape(14, 14).cpu().detach().numpy(), alpha=0.5, cmap='jet', vmin=min, vmax=max)
        plt.axis('off')
...
def plot_attention_map_as_graph(block, fig, attention_maps, image=None):
    '''Plot the image, if provided, and the attention map of each patch of a block as a graph (Flatten the attention map then plot on a graph)'''
    # We get rid of the cls_token
    first_block_attention_maps = attention_maps[0][block][1:, 1:] # dim : (196, 196)
    
    min = first_block_attention_maps.flatten().min()
    max = first_block_attention_maps.flatten().max()

    if image != None:
        plt.imshow(image.permute(1,2,0).cpu().detach().numpy(), cmap='gray', alpha=1)
    for i in range(196):
        fig.add_subplot(14, 14, i+1)
        plt.plot(first_block_attention_maps[i, :].cpu().detach().numpy())
        plt.ylim(min, max)
        plt.axis('off')
...
model_names = ['unetr_depth3_pre_lr_e100', 'unetr_depth6_pre_lr_e100', 'unetr_depth12_pre_lr_e100']
...
all_attention_maps_as_graph_of_block_i = True
cls_token_of_block_i = False
gif_of_rollout_evolution = True
...
for i, model_name in enumerate(model_names):
    ...
    attention_rollout_maps = model.attention_rollout_map(sample_image.unsqueeze(0).to(device=DEVICE), discard_ratio=0.0, head_fusion='max')
    ...
    # Each frame is the attentioin map of block i of all patches overlayed on the sample image
    if gif_of_rollout_evolution:
        depth = kwargs['depth']
        frames = []
        for t in tqdm(range(depth)):
            fig = plt.figure(figsize=(10, 10))
            plt.title(f'Attention map for model {model_name} and block {t}')
            plt.axis('off')
            plot_attention_rollout(t, fig, attention_rollout_maps, sample_image)
            plt.savefig(RESULTS + 'frame/' + model_name + f'_rollout_b{t}' + '.png', bbox_inches='tight')
            frames.append(imageio.v2.imread(RESULTS + 'frame/' + model_name + f'_rollout_b{t}' + '.png'))
            plt.close()
        imageio.mimsave(RESULTS + model_name + f'_rollout' + '.gif', frames, duration=2000, loop=0)

    if all_attention_maps_as_graph_of_block_i:
        fig = plt.figure(figsize=(20, 20))
        plt.title(f'Attention map for model {model_name} and block {blocks[i]} as graph')
        plt.axis('off')
        plot_attention_map_as_graph(blocks[i], fig, attention_maps, sample_image)
        plt.savefig(RESULTS + 'all_attention_maps_as_graph/' + model_name + f'_attn_b{blocks[i]}_as_graph' + '.png', bbox_inches='tight')
